...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f5f5f0', fontFamily:'system-ui, sans-serif' }}>
      <div style={{ background:'#2c1a3e', color:'#fff', padding:'14px 32px',
        display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <span style={{ fontWeight:700, fontSize:16 }}>Church of Christ the Saviour — Admin</span>
        <div style={{ display:'flex', alignItems:'center', gap:20 }}>
          <span style={{ fontSize:13, opacity:0.7 }}>{user?.email}</span>
          <span style={{ fontSize:11,
            background: role === 'superadmin' ? '#d4af37' : '#555',
          ...
          <button onClick={handleSignOut}
            style={{ background:'transparent', border:'1px solid rgba(255,255,255,0.3)',
              color:'#fff', padding:'5px 12px', borderRadius:4, fontSize:13, cursor:'pointer' }}>
            Sign out
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'40px 24px' }}>
        ...
        <p style={{ color:'#666', margin:'0 0 32px' }}>Select an area to manage below.</p>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(200px, 1fr))', gap:16 }}>
          ...
          {role === 'superadmin' && (
            <AdminCard title="Admin Users" description="Manage admin accounts and permissions"
              to="/admin/users" icon="👤" />
          )}
          {role === 'superadmin' && (
            <AdminCard title="Site Settings" description="Phone number, site name, etc."
              to="/admin/settings" icon="⚙️" />
          )}
        </div>

        {role === 'admin' && Array.isArray(allowedSections) && (
          <div style={{ marginTop:32, padding:'16px 20px', background:'#fff',
...
function AdminCard({ title, description, to, icon }:
  { title: string; description: string; to: string; icon: string }) {
  return (
    <Link to={to} style={{ display:'block', textDecoration:'none', background:'#fff',
      border:'1px solid #e0dbd0', borderRadius:6, padding:'22px 20px', transition:'box-shadow 0.15s' }}
